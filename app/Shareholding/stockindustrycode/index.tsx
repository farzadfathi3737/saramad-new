            ...
            <div className="panel h-full w-full px-0">
                <div className="flex h-[3.5rem] items-center justify-between border-b border-gray-300">
                    <div className='p-2 h-full flex items-center pr-5'>
                        <span className="mr-2"> {t('list')} {t('stockindustrycode')}</span>
                    </div>
                ...
                <div className="table-responsive p-5">
                    {modelData && (
                        <Demo
                            ...
                            labaleNameList={[{ label: 'Keyword', value: 'نام صنعت' }]}
                            hideColList={['id', 'companyId', 'companyName']}
                            action={(row) => {
                                return (
                                    <Tooltip label="زیر صنعت">
                                        <ActionIcon
                                            onClick={() => subPage(modelData.name.toString().toLowerCase(), 'substock', undefined, [{ key: 'id', value: row.id }])}
                                            variant="transparent"
                                            className="mr-3 w-9 h-9">
                                            <i className="fa-duotone fa-solid fa-sitemap text-xl text-gray-400 hover:text-orange-500" />
                                        </ActionIcon>
                                    </Tooltip>
                                );
                            }}
