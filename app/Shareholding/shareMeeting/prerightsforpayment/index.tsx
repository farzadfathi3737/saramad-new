...
const Sharecashdividend = () => {
    ...
    useEffect(() => {
        const setdata = async () => {
            ...
            let _model = getEntityModel('sharemeetingprerightsforpayment');
                    ...
                    {modelData && (
                        <Demo
                            ...
                            action={(row) => {
                                return (
                                    <>
                                        <Tooltip label="اسناد پرداخت">
                                            <Link
                                                href={`/Shareholding/shareMeeting/prerightsforpayment/prerightswaivedsell?MeetingId=${rowId}&TradingCodeId=${row.tradingCodeId}&tradingCode=${row.tradingCode}`}
                                        ...
                                        <Tooltip label="ثبت پرداخت">
                                            <Link
                                                href={`/Shareholding/shareMeeting/prerightsforpayment/nominalvaluepayment?MeetingId=${rowId}&TradingCodeId=${row.tradingCodeId}&tradingCode=${row.tradingCode}`}
                                                className="btn btn-outline mr-3 flex items-center rounded-xl bg-secondary-light px-2 font-iranyekan text-secondary"
                                            >
                                                <i className="fa-duotone fa-solid fa-check text-lg mx-1" />
                                            </Link>
